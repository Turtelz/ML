...
class Snake(gym.Env):
    ...
    def __init__(self, render_mode=None, size=20):
        ...
        self.observation_space = spaces.Box(0,3,shape=(self.size * self.size,),dtype=int)
        '''
    ...
    def _get_obs(self):
        # 0 is none, 1 is head, 2 is body, 3 is food
        temp = np.zeros(shape=(self.size,self.size,),dtype=int)
        
        for bod in self.body:
            temp[bod[0]][bod[1]] = 2
        
        temp[self._target_location[0]][self._target_location[1]] = 3
        temp[self._agent_location[0]][self._agent_location[1]] = 1
...
#model = model.load("./ML/models/snake/SnakeBot_10000000_steps")
